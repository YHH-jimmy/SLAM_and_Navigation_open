...
public:
    ...
    bool cachePointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr& laserCloudMsg)
    {
        ...
        {
            deskewFlag = -1;
            timeFieldAvailable = false;
            for (auto &field : currentCloudMsg.fields)
            {
                if (field.name == "time" || field.name == "t")
                {
                    deskewFlag = 1;
                    timeFieldAvailable = true;
                    break;
                }
            }
        }

        if (!timeFieldAvailable && sensor == SensorType::VELODYNE)
        {
            if (computePointTimeVelodyne())
            {
                deskewFlag = 1;
            }
        }
    ...
    bool computePointTimeVelodyne()
    {
        if (laserCloudIn->empty())
            return false;

        cloudInfo.startOrientation = -atan2(laserCloudIn->points.front().y, laserCloudIn->points.front().x);
        cloudInfo.endOrientation = -atan2(laserCloudIn->points.back().y, laserCloudIn->points.back().x) + 2 * M_PI;
        ...
        const float scanPeriod = 0.1f; // 10Hz Velodyne scans for KITTI
    ...
    void projectPointCloud()
    {
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            if (ringFlag == 2) {
                ...
                if (N_SCAN == 16)
                {
                    rowIdn = int((verticalAngle + 15.0f) / 2.0f + 0.5f);
                }
                else if (N_SCAN == 32)
                {
                    rowIdn = int((verticalAngle + 92.0f / 3.0f) * 3.0f / 4.0f);
                }
                else if (N_SCAN == 64)
                {
                    if (verticalAngle >= -8.83f)
                        rowIdn = int((2.0f - verticalAngle) * 3.0f + 0.5f);
                    else
                        rowIdn = N_SCAN / 2 + int((-8.83f - verticalAngle) * 2.0f + 0.5f);

                    if (verticalAngle > 2.0f || verticalAngle < -24.33f || rowIdn > 50 || rowIdn < 0)
                        continue;
                }
                else
                {
                    RCLCPP_ERROR(get_logger(), "Unsupported N_SCAN for Velodyne ring computation: %d", N_SCAN);
                    rclcpp::shutdown();
                }
            }
